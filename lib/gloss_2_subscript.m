...
rmse_gloss_2(ind_outer) = norm(L_g2(:)-Y_gen(:))/sqrt(numel(Yn));
mape_gloss_2(ind_outer) = sum(abs(L_g2-Y_gen),'all')/numel(Yn);
time_gloss = toc
%% Envelope Analysis
...
%% Top-K Analysis
[k_list(:,ind_outer), precision_gloss_2(:,ind_outer), recall_gloss_2(:,ind_outer), fpr_gloss_2(:,ind_outer)] = analyze_top_K(mahal_dist(S_g2), X, ind_removed);
...
if ~nodisp
    ...
    figure,
    plot(k_list, precision_gloss_2,'DisplayName','GLOSS');
    legend, grid
    title('Precision')
    
    figure,
    plot(k_list, recall_gloss_2,'DisplayName','GLOSS');
    legend, grid
    title('Recall')
end