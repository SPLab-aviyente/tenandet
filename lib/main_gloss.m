...
lam_list_2 = (10.^[-3])/(numel(Y));
lam_list_3 = (10.^[-2])/max(size(Y));
gam_list_1 = (10.^[0])/(max(size(Y)));
gam_list_2 = (10.^[1])/(numel(Y));
...
%                         loss_subscript(Yn, Y_gen, X, param);
                    ...
                    param.theta = 0.002;
                    [L_gloss, S_gloss, precision{i}(i_a,:,i_G,i_L,i_N,8),...
                        recall{i}(i_a,:,i_G,i_L,i_N,8),...
                        fpr{i}(i_a,:,i_G,i_L,i_N,8),...
...
save(['preliminary_results\theta_psi_2.mat'], 'recall', 'fpr',...
